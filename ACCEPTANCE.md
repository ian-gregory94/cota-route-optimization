...
| 11 | **Model B discovery adequacy** ⚠️ **CASE C on first pass, 2026-08-26 22:44 UTC** | per-pattern RAPTOR must be shown not to omit a material number of route sequences that become competitive *only* under Model B combined-frequency repricing. Distinct from gate 1: that asks whether the candidate set contains the paths new *headway scenarios* make attractive; this asks whether it contains the paths the corrected *valuation* makes attractive. Thresholds fixed below, before the diagnostic was written. **First pass:** 4.877% of tested flow (material) on 0.185% of tested cost (negligible) — Case C on the flow bound alone. Response as pre-committed: route-level search scenario added to Model B enumeration; rerun pending. The rerun is a consistency check, not an independent test; the frontier's sensitivity to the augmented set is the evidence that counts. `outputs/discovery_adequacy_modelB.json`. |
