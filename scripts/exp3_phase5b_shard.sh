#!/usr/bin/env bash
# One Phase 5b worker -- the symmetric escalation of the six promoted states
# that section 6 never triggered on. Identical regime to the section 6 batch
# (contract 45e23ae01be4d071, 40 restarts, five predeclared seeds) writing into
# the SAME observation store, because it is the same regime. Reads its own
# manifest; the frozen section 6 manifest is untouched. Slice budget is the
# re-sized one proven over the last 30 cells of that batch.
#
...
while true; do
  ...
  git diff --cached --quiet || git commit -q -m "chore: Experiment 3 Phase 5b symmetric escalation cells, shard $SHARD
